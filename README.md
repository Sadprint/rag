...
## 效果演示
以下内容为了演示，提前预设了读取t2ranking的数据作为知识库。ai将会从这中间读取知识加强回答：
![对话1](/images/对话1.png)
左侧是ai根据知识库做出的回答，右侧是ai调用的知识库。可以看到ai能正确的读取到对应的内容。还能根据文档做出正确的回答

![对话2](/images/对话2.png)
示例问答过程，ai能正确根据知识库做出回答。还能看到llm会将原始问题修改成更好的问题再输入，提高命中率。

![对话3](/images/对话3.png)
对于进一步更加详细的要求也能够理解。右侧能看到ai调用了更多的文档。
...
用户的问题有两种情况无法直接检索到匹配文档：

| 用户原始问题 | 问题类型 | 原因 |
|-------------|------|------|
| "心肌酶组套是什么意思" | — | 陈述式关键词查询，能匹配 |
| "你知道心肌酶组套吗？" | **问句形式转换** | 向量捕获的是"询问是否知道X"而非"X的定义"，噪声词"你知道"/"吗"稀释了关键词信号 |
| "我做的 rag 为什么会这样" | **指代消解** | "这样"是指代词，"我做的"是口语化表述，只有结合对话历史才能知道在问什么 |

向量检索只做语义相似度匹配，**不理解对话上下文，也不会自动标准化问句形式**。

**解决思路：LLM 改写（Query Rewriting）**

在检索之前加一个轻量步骤：用 LLM 把用户问题改写成适合检索的关键词查询。

```
── 情形1：问句形式转换（每条都做）──
用户: "你知道心肌酶组套吗？"      ← yes/no 问句，语义漂移
              ↓  LLM 改写
       "心肌酶组套 定义 介绍"          ← 关键词查询，精准匹配
              ↓  ChromaDB 检索
         检索到相关文档 → LLM 生成回答
...
用户: "RAG 系统召回率很低怎么办"
助手: "可以尝试调整 MMR 参数..."
用户: "我做的 rag 为什么会这样"    ← 依赖上下文
              ↓  LLM 改写
       "RAG 系统召回率低的原因是什么" ← 独立查询
              ↓  ChromaDB 检索
         检索到相关文档 → LLM 生成回答
```
...
核心原则：**始终改写**（不再限制"无历史则跳过"），改写 prompt 根据有无历史动态调整规则。

1. **`rag_chain.py` — `rewrite_query()` 函数**
   - **始终运行**：不再跳过首条消息，每条用户问题都过改写
   - **两套规则合并**：基础规则（问句→关键词）始终生效；指代消解规则仅在存在对话历史时附加
   - **异常回退**：改写失败返回原始问题，不阻塞主流程

2. **`rag_chain.py` — `build_rag_chain()` 增加 `query` 参数**
   - 提供改写查询时，通过 `RunnableLambda` 包装检索器，使之使用改写后的查询
   - LLM 的 prompt 中 `question` 字段仍使用用户原始输入
   - **检索用改写查询，生成用原始问题**

3. **`app.py` — 在检索前调用改写**
   - `rewrite_query(message.content, history)` → 改写后的查询
   - 改写后查询传入 `build_rag_chain(query=rewritten_query)` 和 `retrieve_with_sources()`
   - 改写结果自动展示在侧边栏"查询改写"元素中（仅当被改写时）
